...
<form role="search" class="search-form" method="get" action="<?php echo esc_url( home_url( '/' ) ); ?>" <?php echo $aria_label; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- Escaped above. ?>>
	...
	<div class="input-group">
		...
		<?php if ( 'bootstrap5' === $bootstrap_version ) : ?>
			<button type="submit" class="submit search-submit btn btn-secondary" name="submit"><i class="fa fa-search"></i></button>
		<?php else : ?>
			<span class="input-group-append">
				<input type="submit" class="submit search-submit btn btn-secondary" name="submit" value="<?php echo esc_attr_x( 'Search', 'submit button', 'understrap' ); ?>">
			</span>
		<?php endif; ?>
	</div>
</form>
